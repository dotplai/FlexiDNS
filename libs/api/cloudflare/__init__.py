...
class CloudFlare:
    ...
    def get_zone_id(self: Self, domain: str) -> str:
        ...
        if not response.ok:
            logger.exception(ConnectionError(f"Error fetching zone id for domain '{domain}'. Response Code: {response.status_code}, Error: {response.text}"))
    ...
    def get_record_id(self: Self, zone_id: str, record_name: str) -> str:
        ...
        if not response.ok:
            logger.exception(ConnectionError(f"Error fetching DNS record ID for '{record_name}'. Response Code: {response.status_code}, Error: {response.text}"))
        ...
        if data['success'] and data['result']:
            ...
            logger.exception(ConnectionRefusedError("DNS record not found or API call unsuccessful."))

    def get_dns_record(self: Self, zone_id: str, filter: object = None) -> dict:
        """
    ...
    def A(self: Self, fqdn: str, content: str, ttl: int = None, proxied: bool = None, comment: str = None) -> None:
        ...
        old_record: dict = self.get_dns_record(zone_id, {"name": fqdn})
        if not old_record:
            logger.exception(KeyError(f"No domain name '{fqdn}' found in your DNS records. Please create it first."))
        ...
        if not response.ok:
            logger.exception(ConnectionError(f"Error updating DNS record. Response Code: {response.status_code}, Error: {response.text}"))

        if not result.get("success", False):
            logger.exception(ConnectionRefusedError(f"Failed to update content of '{fqdn}' to '{content}'. Errors: {result['errors']}"))
